...
def test_radar_dual_line_cann_reanchors_after_long_outage():
    ...
    assert diagnostics[-2]["recovery_pending"]
...
def test_radar_recovery_confirmation_rejects_two_opposite_faults():
    ...
    measurement[30] += [3_000.0, 5.0]
    measurement[31] -= [3_000.0, 5.0]
    filtered, diagnostics = _radar_range_rate_line_cann(
        timestamps, measurement, available,
    )
    assert all(diagnostics[index]["recovery_pending"]
               for index in (30, 31, 32))
    assert diagnostics[33]["radar_reanchored"]
    assert np.allclose(filtered[33], measurement[33], atol=1e-6)
