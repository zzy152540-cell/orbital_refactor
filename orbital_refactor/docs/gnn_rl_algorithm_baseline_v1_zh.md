...
随后在完全相同的条件440～459上完成随机初始化与暖启动PPO的首个20条件完整周期，
其中五星8回合、10星7回合、20星5回合。投入全新的开发条件460、461、462、463、
464和466后，随机初始化总体平均改善约0.01365 m，5/6回合改善，最差退化约
0.01044 m；暖启动总体平均改善约0.02132 m，4/6回合改善，但最差退化扩大到
0.02990 m。暖启动在两个20星条件上明显更强，却在两个五星条件上均退化。两种
确定性策略仍只选择add/keep，尽管随机训练轨迹中已出现swap/remove；Critic解释
方差仍较低，随机与暖启动分别约0.014和0.009。因此暖启动提高了跨规模平均值，
但尚不能认定为更安全的初始化。20回合单批次只产生一次PPO更新，下一轮应采用
多个rollout/update批次，让更新后的策略参与后续经验采集。上述6个开发条件现已
消耗，不得再用于模型选择或调参。

事后网络结构审查发现，上述历史随机初始化对照使用
`explicit_action_pairing=False`，而监督暖启动checkpoint使用`True`，因此该20条件
结果只能作为探索性证据，不能解释为“仅初始化不同”的严格消融。混合规模PPO现已
把配对编码开关纳入冻结配置，随机与暖启动默认都使用`True`；如果checkpoint结构
与配置不一致则直接拒绝训练。回归测试进一步要求两种初始化Actor的参数键与张量
形状完全相同。后续多批次比较必须使用修正后的结构基线；已消耗的440～466结果
不会重跑或重新标记为严格对照。

在Actor结构严格对齐后，使用训练条件500～559完成60回合、每10回合更新一次的
六批次对照，并首次投入条件580、581、582、583、584和586。随机初始化最终在
6/6评价回合全部确定性选择keep，与keep基线完全一致。暖启动总体平均改善约
0.02081 m，但只有3/6回合改善，最差退化约0.01654 m；五星2/2明显改善，10星
0/2改善，20星仅1/2改善。暖启动确定性策略仍只有add/keep，没有swap/remove。
两分支Critic解释方差均随批次下降，最终约为-0.11。由此确认多批次消除了旧实验
“只更新一次”的限制，却没有解决跨规模信用分配；继续增加回合数前，应先诊断按
规模划分的value target、Critic预测和动作类型优势。条件580～586现已作为开发
证据消耗。

进一步加载冻结模型，在全新条件600、601、602、603、604和606上比较Critic预测与
不依赖Critic的折扣Monte Carlo return。两分支return标准差约0.67，而预测value
标准差仅约0.06～0.07；解释方差约-0.12，target/value相关系数为负。按决策序号
分组后，平均return从决策0的约2.18下降到决策9的约-0.03，但随机Critic预测从
0.267上升到0.416，暖启动Critic从0.210上升到0.342。故问题并非只有不同星数的
均值偏移，而是Critic学反了时域方向。变规模训练现已把人为截取的训练窗口末端标为
truncated，并对最终value进行bootstrap；旧实验配置默认仍保持terminated语义。
这项修正只改变回报构造，不改变环境物理过程和奖励函数。还需使用全新条件完成受控
重训，才能判断Critic校准是否改善；在此之前不增加“剩余时域”特征或扩大网络。
条件600～606现已作为诊断证据消耗。

随后在全新条件620～649上完成30回合、每10回合更新一次的受控重训，并在
680～686上评价策略、在700～706上审计冻结Critic。训练内解释方差仍下降到约
-0.04；独立审计解释方差约为随机-0.104、暖启动-0.099，target/value相关系数
分别约为-0.62和-0.69。暖启动在6个评价回合中改善5个，平均改善约0.00337 m，
随机初始化仍退化为全keep。由此可见，truncation是持续训练窗口的正确语义，但
不足以表达滤波器从初始化到收敛所产生的强回合相位。下一项最小消融应只向Critic
提供归一化回合进度，不改变Actor、奖励函数或环境物理过程。

随后在条件720～749上完成严格配对的30回合Critic-only相位消融。相位采用独立
单维投影并零初始化，基础Critic权重逐项保持一致，第一批解释方差也完全相同。
相位提示后续没有改善：随机初始化最终解释方差由基线-0.03482变为-0.03541，
暖启动由-0.04899变为-0.04983；确定性策略结果也完全一致。由于Critic原本已经
看到`log1p_timestamp`，额外线性时间表示不是主要瓶颈。后续不再扩展同类时间
特征，下一步应区分滤波初始化瞬态奖励与拓扑动作相关的回报目标。条件720～749及
780～786现已作为开发证据消耗。

在条件820～825上进一步进行同状态、同噪声的一步反事实奖励审计，将始终keep时的
滤波改善与拓扑动作增量分离。keep奖励RMS为0.5037 m，所有非keep相对keep增量
RMS仅0.02084 m，占4.14%；10星和20星分别只有2.56%和1.67%。决策0的keep
改善均值高达1.485 m。监督参考策略平均增量仅0.00186 m，正收益率55%，但受限
候选中的oracle最佳非keep动作有86.7%为正。因此当前绝对RMSE奖励确实主要由滤波
初始化瞬态支配。下一项受控训练应采用“所选动作奖励减去同状态keep反事实奖励”，
同时继续用绝对任务指标评价最终性能。

随后在已消耗的条件720～749上进行30回合配对探索训练。暖启动Critic三批解释
方差由绝对奖励的0.023/-0.013/-0.049变为差分奖励的0.115/0.435/0.338，确定性
评价平均RMSE改善由0.00211 m增至0.00455 m。在全新条件920～926上的冻结审计中，
总体解释方差为随机0.726、暖启动0.519，target/value相关系数为0.855和0.722。
但按单一规模划分后，10星和20星解释方差仍为负，暖启动仅五星为正。因此反事实
差分奖励是值得继续确认的开发选项，尚不替换正式默认：它消除了公共滤波瞬态并
恢复总体value结构，但同规模内的动作信用分配仍未解决。

在全新训练条件940～969和评价条件1000～1006上的配对确认仍保留暖启动策略收益：
平均RMSE改善由绝对奖励0.00431 m增至差分奖励0.00703 m，主要来自20星场景由
0.00671 m增至0.01488 m；训练内Critic最终解释方差由-0.033变为0.225。但在
全新条件1040～1046上的冻结审计未确认value校准：暖启动总体解释方差为-0.243
（绝对奖励为-0.080），尽管相关系数由-0.616改善为+0.211，且因差分回报尺度
显著缩小，value RMSE由0.813降至0.0215。各单一规模解释方差仍全部为负。因此
差分奖励继续保持可选，下一问题是微小差分回报在不同星数间的中心化与尺度校准，
而不是扩展Actor容量。

随后在同一组940～969开发条件上测试固定分规模RMS归一化，使用独立审计得到的
5/10/20星非keep增量RMS 0.02464/0.01542/0.00745 m，并同步缩放任务增量与资源
惩罚。该朴素方案失败：暖启动最终解释方差由未缩放0.225降至-0.0148，value loss
由约5e-4升至1.48，确定性平均RMSE改善由0.00703 m降至0.00542 m。该功能仅保留
为默认关闭的负消融。后续应在Critic端标准化value target或按规模校准value，
而不改变Actor所使用的物理奖励与成本权衡。

随后测试恒等初始化的分规模Critic仿射头，不改变奖励与Actor输入。在940～969配对
训练中，暖启动最终解释方差由0.225提高到0.286，确定性策略完全不变。冻结审计
重复确认总体跨规模校准收益：1040～1046上暖启动EV由-0.243升至-0.007，
全新1080～1086上由0.153升至0.292；相关系数也分别由0.211升至0.358、由
0.521升至0.595。各单一规模EV仍为负且基本不变。因此该仿射头可作为推荐的可选
跨规模Critic校准结构，但不能视为同规模动作信用分配方案，也尚未产生策略收益。

随后使用Critic实际采用的池化状态特征进行同规模held-out线性审计：训练条件
1120～1134、测试条件1180～1186，ridge惩罚扫描0.1～100。较强正则下三个规模
均恢复正的测试结构：五星最佳EV/相关为0.507/0.740（全局特征），10星为
0.623/0.789（完整特征），20星为0.383/0.627（完整特征）；仅时间特征在五星和
...
JSON同时保存逐episode诊断，旧结果则自动使用分规模聚合训练数据。训练可继续使用
`state_estimate_gnn`环境；绘图可使用已安装Matplotlib的`state_estimate`环境。
