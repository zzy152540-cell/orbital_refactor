...
def run_single_satellite_cann_comparison(
    *, duration: float = 1800.0, dt: float = 2.0, seed: int = 0,
    ...
    available = ~((timestamps >= outage_start) & (timestamps <= outage_end))
    ...
    phase_error = (cann.decoded_phase - truth_phase + np.pi) % (2 * np.pi) - np.pi
    ...
    return {
        ...
        "cann": cann, "available": available,
        "summary": {
            "position_rmse_m": float(np.sqrt(np.mean(position_error**2))),
            "velocity_rmse_mps": float(np.sqrt(np.mean(velocity_error**2))),
            "cann_phase_rmse_deg": float(np.rad2deg(np.sqrt(np.mean(phase_error**2)))),
            "cann_max_phase_error_deg": float(np.rad2deg(np.max(np.abs(phase_error)))),
            "cann_valid_fraction": float(np.mean(cann.valid)),
...
def write_single_satellite_cann_results(result, output_dir: str | Path) -> dict[str, Path]:
    ...
    t = result["timestamps"]
    cann = result["cann"]
    phase_error = (cann.decoded_phase - result["truth_phase"] + np.pi) % (2*np.pi) - np.pi
    ...
    axes[2].set_ylabel("CANN diagnostic"); axes[2].set_xlabel("Time (s)")
    ...
    fig.suptitle("Single-satellite three-modal Federated-CI with passive CANN")
    fig.tight_layout()
