...
def plot_results(t, results_dict):
    ...
    for name, res in results_dict.items():
        plt.plot(t, np.linalg.norm(res["pos_err"], axis=1), label=name)
    plt.xlabel("Time [s]")
    ...
    plt.figure(figsize=(10, 6))
    for name, res in results_dict.items():
        plt.plot(t, np.linalg.norm(res["vel_err"], axis=1), label=name)
    plt.xlabel("Time [s]")
    ...
    plt.figure(figsize=(10, 6))
    for name, res in results_dict.items():
        plt.plot(t, np.linalg.norm(res["target_acc_err"], axis=1), label=name)
    plt.xlabel("Time [s]")
    ...
    for name, res in results_dict.items():
        ...
        if gate_hist is not None and np.any(gate_hist):
            idx = np.where(gate_hist)[0]
            plt.scatter(t[idx], res["nis"][idx], s=12, marker="x")
    plt.xlabel("Time [s]")
    plt.ylabel("NIS")
