...
CONFIG = {
    ...
    # ===== 推理结果 =====
    "pred_npz_path": r"E:\Python Files\Thesis_Code\infer_outputs\checkpoints_pose_geo_dist_auto\roe1\predictions.npz",
...
def run_federated_ci_filter(
    ...
    return {
        ...
        "nis": nis_hist,
        "gate_hist": gate_hist,
        "valid_nn": np.asarray(valid_nn, dtype=bool),
...
def export_results_to_csv(save_path: str, t: np.ndarray, res: Dict[str, Any]):
    ...
    nis_nn = np.asarray(res["nis"]["nn"], dtype=float).reshape(-1)
    nis_ir = np.asarray(res["nis"]["ir"], dtype=float).reshape(-1)
    nis_rad = np.asarray(res["nis"]["rad"], dtype=float).reshape(-1)

    gate_nn = np.asarray(res["gate_hist"]["nn"], dtype=np.uint8).reshape(-1)
    ...
    valid_nn = np.asarray(res["valid_nn"], dtype=np.uint8).reshape(-1)
    ...
    data_mat = np.column_stack([
        ...
        nis_nn, nis_ir, nis_rad,
        gate_nn, gate_ir, gate_rad,
        valid_nn, valid_ir, valid_rad,
        w_nn, w_ir, w_rad,
    ])
    ...
    header = (
        ...
        "gate_nn,gate_ir,gate_rad,"
        "valid_nn,valid_ir,valid_rad,"
        "ci_w_nn,ci_w_ir,ci_w_rad"
    )
    np.savetxt(save_path, data_mat, delimiter=",", header=header, comments="", fmt="%.10e")
    print(f"[INFO] 已导出 NN+IR+RAD 联邦CI 时序 CSV 文件: {save_path}")
...
def export_rmse_report_csv(save_path: str, traj_name: str, res: Dict[str, Any]):
    ...
    with open(save_path, "w", encoding="utf-8", newline="") as f:
        ...
        f.write(
            f"{traj_name},federated_ci_nn_ir_rad,"
    ...
    print(f"[INFO] 已导出 NN+IR+RAD 联邦CI RMSE 报告文件: {save_path}")
...
def main():
    ...
    csv_path = cfg["export_csv_path"] if cfg.get("export_csv_path") else f"{cfg['traj_name']}_federated_ci_nn_ir_rad_results.csv"
    rmse_path = cfg["export_rmse_report_path"] if cfg.get("export_rmse_report_path") else f"{cfg['traj_name']}_federated_ci_nn_ir_rad_rmse_report.csv"
