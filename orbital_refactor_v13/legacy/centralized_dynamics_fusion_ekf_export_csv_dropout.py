...
def export_rmse_report_csv(save_path: str, traj_name: str, res: Dict[str, Any]):
    ...
    print(f"[INFO] 已导出集中式 RMSE 报告文件: {save_path}")
...
def main():
    ...
    csv_path = cfg["export_csv_path"] if cfg.get("export_csv_path") else f"{cfg['traj_name']}_centralized_results.csv"
    rmse_path = cfg["export_rmse_report_path"] if cfg.get("export_rmse_report_path") else f"{cfg['traj_name']}_centralized_rmse_report.csv"
