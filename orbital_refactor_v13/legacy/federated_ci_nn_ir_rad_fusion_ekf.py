...
CONFIG = {
    ...
    # ===== 推理结果 =====
    "pred_npz_path": r"E:\Python Files\Thesis_Code\infer_outputs\roe1\predictions.npz",
...
def run_federated_ci_filter(
    ...
    for k in range(1, N):
        ...
        if len(posterior_list) == 0:
            ...
            ci_weight_hist.append([])
        elif len(posterior_list) == 1:
            x_fused[k] = posterior_list[0][1].copy()
            P_fused[k] = posterior_list[0][2].copy()
            ci_weight_hist.append([])
        else:
            sort_key = {"rad": 0, "nn": 1, "ir": 2}
            posterior_list_sorted = sorted(posterior_list, key=lambda x: sort_key[x[0]])
            x_ci, P_ci, weights = ci_fuse_multiple(
                posterior_list_sorted, objective=ci_objective_name, grid_points=ci_grid_points
            )
            x_fused[k] = x_ci
            P_fused[k] = P_ci
            ci_weight_hist.append(weights)
...
def plot_results(t, res, t_start_plot=500.0):
    ...
    # 5. CI 权重可视化
    # 说明：
    # ci_weight_hist 往往是在主循环 k=1...N-1 时记录的，
    # 因而其长度通常比 len(t) 少 1。
    # 这里通过 start=1 对齐到时间轴。
    # -----------------------------
    ci_weight_hist = res["ci_weight_hist"]

    w_pair1 = np.full(len(t), np.nan)   # 第一次 pair CI 融合权重
    w_pair2 = np.full(len(t), np.nan)   # 第二次 pair CI 融合权重（若存在）

    label_pair1 = None
    label_pair2 = None

    for k, wk in enumerate(ci_weight_hist, start=1):
        # wk 可能为 [] / [(label, w)] / [(label1, w1), (label2, w2)]
        if len(wk) >= 1:
            if label_pair1 is None:
                label_pair1 = wk[0][0]
            w_pair1[k] = wk[0][1]

        if len(wk) >= 2:
            if label_pair2 is None:
                label_pair2 = wk[1][0]
            w_pair2[k] = wk[1][1]

    w_pair1_plot = w_pair1[mask]
    w_pair2_plot = w_pair2[mask]

    plt.figure(figsize=(10, 6))
    has_curve = False

    if np.any(~np.isnan(w_pair1_plot)):
        plt.plot(
            t_plot,
            w_pair1_plot,
            label=f"CI weight 1 ({label_pair1 if label_pair1 is not None else 'pair1'})"
        )
        has_curve = True

    if np.any(~np.isnan(w_pair2_plot)):
        plt.plot(
            t_plot,
            w_pair2_plot,
            label=f"CI weight 2 ({label_pair2 if label_pair2 is not None else 'pair2'})"
        )
        has_curve = True
