...
def gen_ir_measurements(
    ...
    z_ir = np.column_stack([az_meas, el_meas])
...
# =========================
class EKF:
    def __init__(self, F, Q, R, h_func, H_func, mode_name="generic"):
        self.F = F
        self.Q = Q
        self.R = R
        self.h_func = h_func
    ...
    def update(self, x_pred, P_pred, z):
        z_pred = self.h_func(x_pred)
        H = self.H_func(x_pred)

        y = measurement_residual(z, z_pred, self.mode_name)

        S = H @ P_pred @ H.T + self.R
        S = S + 1e-9 * np.eye(S.shape[0])   # 数值正则化
...
def run_single_modal_filter(
    ...
    for k in range(1, N):
        ...
        if valid_all[k]:
            x, P, _, _, _, nis = ekf.update(x_pred, P_pred, z_all[k])
            nis_hist[k] = nis
        else:
            x, P = x_pred, P_pred
...
def main():
    ...
    # 红外噪声（弧度）
    sigma_az = np.deg2rad(0.3)
    sigma_el = np.deg2rad(0.3)
    ...
    z_ir, valid_ir = gen_ir_measurements(
        r_true, sigma_az=sigma_az, sigma_el=sigma_el, dropout_prob=0.0, rng=rng
    )
    ...
    ekf_ir = EKF(F, Q, R_ir, h_ir, H_ir, mode_name="ir")
    ...
    res_opt = run_single_modal_filter(
        t, r_true, v_true, z_opt, valid_opt, ekf_opt, x0, P0
    )
    res_ir = run_single_modal_filter(
        t, r_true, v_true, z_ir, valid_ir, ekf_ir, x0, P0
    )
    res_rad = run_single_modal_filter(
        t, r_true, v_true, z_rad, valid_rad, ekf_rad, x0, P0
    )
